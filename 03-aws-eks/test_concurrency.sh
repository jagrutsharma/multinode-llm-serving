...
# Ray Serve actually spreads load across both GPU-backed QwenChat replicas or piles
# everything onto one -- same test as phase 2, now against two separate g4dn.xlarge
# nodes instead of two CPU pods on one local machine.
#
...
# some ~2x) indicate real load-spreading across the two replicas. Durations that form one
# steadily increasing staircase indicate everything landed on a single replica.
#
# For ground truth on which replica (and which GPU node) handled which request, check the Ray
# dashboard (kubectl port-forward svc/rayservice-sample-head-svc 8265:8265, then Serve ->
# QwenChat -> each replica -> Logs) and compare POST /generate completions per replica ID.
...
# Open tunnel from local port 8100 to RayService's dedicated Serve Service on port 8000
# (rayservice-sample-serve-svc -- the Service that always points at whichever pods are
# currently serving traffic, not necessarily just the head). & backgrounds it so the
# tunnel doesn't block the rest of the script.
kubectl port-forward svc/rayservice-sample-serve-svc "${LOCAL_PORT}:8000" > "${OUT_DIR}/port-forward.log" 2>&1 &
...
while IFS= read -r p; do
  ...
  (
    ...
    # get the unix timestamp (seconds) - after response
    end=$(date +%s)
