...
The comparison itself is the real story. I expected a modest gap — a 0.5B model seemed too small for batching to matter much. It wasn't modest. At 16 concurrent requests, vLLM delivered ~27x the throughput of plain Transformers, latency flat under a second while the old setup's climbed past 13. Smaller models turn out to benefit *more* from batching, not less — fixed per-request overhead matters more when the model's own compute cost is tiny.
